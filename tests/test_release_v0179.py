...
class ReleaseV0179Tests(unittest.TestCase):
    def test_unified_app_compiles_and_reports_v0179(self):
        src=text('server/unified_app.py')
        ast.parse(src,filename='server/unified_app.py')
        self.assertIn("UI_VERSION='0.17.9'",src)
        self.assertIn("'version':'0.17.9'",src)

    def test_ui_health_reports_hardening_features(self):
        src=text('server/unified_app.py')
        for marker in (
    ...
    def test_readme_publishes_v0179_and_locked_control(self):
        readme=text('README.md')
        self.assertTrue(readme.startswith('# Stock Day Trader v0.17.9'))
