...
if('serviceWorker' in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('/sw.js?v=141').catch(()=>{}));
const liveClassic=location.pathname==='/classic'||location.pathname.startsWith('/classic/');
if(liveClassic){import('./ui-polish.js?v=141').catch(()=>{});import('./live-app.js?v=141').catch(showBootError)}else import('./app.js').catch(showBootError);
