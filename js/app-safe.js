...
if('serviceWorker' in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('/sw.js?v=175').catch(()=>{}));
const liveClassic=location.pathname==='/classic'||location.pathname.startsWith('/classic/');
if(liveClassic){import('./ui-polish.js?v=175').catch(showBootError);import('./live-app.js?v=175').catch(showBootError);import('./trade-name-fix.js?v=175').catch(showBootError)}else import('./app.js?v=175').catch(showBootError);
