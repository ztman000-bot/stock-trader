export function renderChart(canvas,history,indicators){
 ...
 canvas.width=w*dpr;canvas.height=h*dpr;ctx.scale(dpr,dpr);ctx.clearRect(0,0,w,h);if(history.length<2)return;
 const data=history.slice(-70),prices=data.flatMap(x=>[x.high,x.low,indicators.vwap]),min=Math.min(...prices),max=Math.max(...prices),pad=(max-min)*.08||1,lo=min-pad,hi=max+pad;
 ...
 emaLine(9,'#ffd166');emaLine(20,'#6ea8fe');ctx.setLineDash([5,4]);ctx.strokeStyle='#b58cff';ctx.beginPath();ctx.moveTo(34,y(indicators.vwap));ctx.lineTo(w-10,y(indicators.vwap));ctx.stroke();ctx.setLineDash([]);
 ctx.fillStyle='#8ea0b9';ctx.font='10px system-ui';ctx.fillText(`EMA9 ${Math.round(indicators.ema9||0).toLocaleString()} · EMA20 ${Math.round(indicators.ema20||0).toLocaleString()} · VWAP ${Math.round(indicators.vwap||0).toLocaleString()} · RSI ${indicators.rsi.toFixed(1)} · ADX ${indicators.adx.toFixed(1)}`,34,h-8);
}
