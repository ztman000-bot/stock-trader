...
ENABLE_TRADING = False  # hard lock during validation
...
PAPER_ENTRY_START = os.getenv('PAPER_ENTRY_START', '09:30')
PAPER_ENTRY_CUTOFF = os.getenv('PAPER_ENTRY_CUTOFF', '14:50')
PAPER_EOD_EXIT = os.getenv('PAPER_EOD_EXIT', '15:15')
...
BACKFILL_MAX_CODES = max(20, min(int(os.getenv('BACKFILL_MAX_CODES', '140')), 200))
...
_PAPER_STATUS = {
    'running': False, 'startedAt': None, 'lastCycleAt': None, 'lastSignalBar': {},
    'lastError': None, 'entries': 0, 'closed': 0, 'shadowSignals': 0,
    'eodExits': 0, 'staleSignals': 0,
}
_PAPER_THREAD = None
...
def _safe_error(e):
    if isinstance(e, NhplugError):
        return {
            'category': getattr(e, 'category', 'nhplug'),
            'code': getattr(e, 'code', ''),
...
def _backfill_one(code: str, count: int = BACKFILL_COUNT):
    ...
    data = nh_call('/krstock/quote/v1/period', {
        'market_cd': 'KRX', 'iem_cd': code, 'edate': now.strftime('%Y%m%d'),
        'array_cnt': str(count), 'gubun': '5', 'xtick': '5',
        'today_cls_code': '1', 'fake_tick': '1',
    })
    ...
    with sqlite3.connect(DB_PATH, timeout=10) as conn:
        ...
        for r in rows:
            ...
            try:
                ...
                if bucket >= current_bucket or not (9 * 60 <= hm <= 15 * 60 + 30):
                    skipped += 1
                    continue
                ...
                skipped += 1
                continue
            conn.execute('''INSERT INTO bars_5m(code,bucket,open,high,low,close,volume,sample_count)
                            VALUES(?,?,?,?,?,?,?,0)
                            ON CONFLICT(code,bucket) DO UPDATE SET
                              open=excluded.open,high=excluded.high,low=excluded.low,
                              close=excluded.close,volume=excluded.volume,sample_count=0''',
...
def run_backfill(codes=None, count: int = BACKFILL_COUNT):
    # The collector verifies the official NH master in the background. Wait briefly so
    # startup backfill covers the expanded safe universe rather than only the fallback list.
    if not codes:
        collector.wait_for_universe(timeout=20)
    target = list(codes or collector.watchlist)[:BACKFILL_MAX_CODES]
    _BACKFILL_STATUS.update({
        'running': True,
        'lastRunAt': datetime.now(KST).isoformat(),
        'lastError': None,
        'results': {},
    })
    errors = []
...
def _entry_hours(now):
    ...
    return _hm(PAPER_ENTRY_START) <= hm < _hm(PAPER_ENTRY_CUTOFF)


def _eod_due(now):
    hm = now.hour * 60 + now.minute
    return now.weekday() < 5 and _hm(PAPER_EOD_EXIT) <= hm < 15 * 60 + 20
...
def _paper_loop():
    ...
    while not _PAPER_STOP.is_set():
        try:
            now = datetime.now(KST)
            _PAPER_STATUS['lastCycleAt'] = now.isoformat()
            marked = mark_positions()
            _PAPER_STATUS['closed'] += len(marked.get('closed', []))
            if _eod_due(now):
                eod = force_close_all('EOD_EXIT')
                _PAPER_STATUS['closed'] += len(eod)
                _PAPER_STATUS['eodExits'] += len(eod)
            elif _entry_hours(now):
                for ev in scan():
                    code = ev['code']
                    ind = ev.get('indicators') or {}
                    bucket = ind.get('bucket')
                    if not bucket or ev['action'] not in ('BUY_CANDIDATE', 'SHADOW_ONLY'):
                        continue
                    if _PAPER_STATUS['lastSignalBar'].get(code) == bucket:
                        continue
                    if not _signal_fresh(bucket, now):
                        _PAPER_STATUS['lastSignalBar'][code] = bucket
                        _PAPER_STATUS['staleSignals'] += 1
                        continue
                    result = paper_enter(code, PAPER_CAPITAL, evaluation=ev)
                    _PAPER_STATUS['lastSignalBar'][code] = bucket
                    if result.get('ok') and result.get('shadow'):
                        _PAPER_STATUS['shadowSignals'] += 1
                    elif result.get('ok'):
                        _PAPER_STATUS['entries'] += 1
...
def start_paper_loop():
    ...
    _PAPER_STOP.clear()
    _PAPER_THREAD = threading.Thread(target=_paper_loop, name='paper-trading-loop', daemon=True)
    _PAPER_THREAD.start()
    return dict(_PAPER_STATUS)
...
def stop_paper_loop():
    _PAPER_STOP.set()
    if _PAPER_THREAD and _PAPER_THREAD.is_alive():
        _PAPER_THREAD.join(timeout=5)
    return dict(_PAPER_STATUS)


@asynccontextmanager
async def lifespan(app: FastAPI):
    if _credentials_ready() and AUTO_START_COLLECTOR:
        collector.start()
    if _credentials_ready() and AUTO_BACKFILL:
        run_backfill()
    if AUTO_PAPER:
        start_paper_loop()
    yield
...
def health():
    return {
        ...
        'entryStart': PAPER_ENTRY_START,
        'entryCutoff': PAPER_ENTRY_CUTOFF,
        'eodExit': PAPER_EOD_EXIT,
        'signalMaxAgeSec': SIGNAL_MAX_AGE_SEC,
        'backfill': dict(_BACKFILL_STATUS),
        'collector': collector.status(),
        'universe': universe_status(),
        'paperLoop': dict(_PAPER_STATUS),
        'paper': daily_stats(),
        'risk': {
            'maxOpenPositions': MAX_OPEN_POSITIONS,
            'roundTripCostEstimatePct': round(ROUND_TRIP_COST_EST * 100, 3),
            'costCoverProtectPct': round(BREAKEVEN_BUFFER_PCT * 100, 3),
        },
    }
...
def _mobile_payload():
    evaluations = scan()
    codes = [e['code'] for e in evaluations]
    quotes = latest_quotes(codes)
    pos = open_positions()
    qmap = {q['code']: q for q in latest_quotes([p['code'] for p in pos])}
    enriched = []
    for p in pos:
        q = qmap.get(p['code'], {})
        current = float(q.get('price') or p['entry_price'])
...
def nh_test():
    ...
    try:
        return {
            'ok': True,
            'code': '005930',
            'message': 'NH PLUG 인증 및 실제 시세 조회 성공',
            'data': nh_call('/krstock/quote/v1/currentPrice', {'iem_cd': '005930', 'market_cd': 'KRX'}),
        }
...
def current_quote(code: str):
    _validate_code(code)
    return {'ok': True, 'code': code, 'data': nh_call('/krstock/quote/v1/currentPrice', {'iem_cd': code, 'market_cd': 'KRX'})}
...
def collector_start(codes: str | None = Query(default=None)):
    ...
    return {'ok': True, 'collector': collector.start(parsed)}


@app.post('/api/collector/stop')
def collector_stop():
    return {'ok': True, 'collector': collector.stop()}


@app.get('/api/collector/status')
def collector_status():
    return {'ok': True, 'collector': collector.status(), 'universe': universe_status()}
...
def market_latest():
    return {'ok': True, 'rows': latest_quotes(collector.watchlist)}


@app.get('/api/market/active')
def market_active(limit: int = 30):
    return {'ok': True, 'rows': active_candidates(limit), 'universe': universe_status()}
...
def paper_scan():
    return {'ok': True, 'rows': scan(), 'daily': daily_stats(), 'universe': universe_status()}
...
def paper_entry(code: str, capital: float = PAPER_CAPITAL):
    _validate_code(code)
...
def paper_positions():
    return {'ok': True, 'open': open_positions(), 'daily': daily_stats(), 'loop': dict(_PAPER_STATUS)}


@app.post('/api/paper/loop/start')
def paper_loop_start():
    return {'ok': True, 'loop': start_paper_loop()}


@app.post('/api/paper/loop/stop')
def paper_loop_stop():
    return {'ok': True, 'loop': stop_paper_loop()}


@app.post('/api/nh/order')
def order_locked():
    raise HTTPException(423, 'Engine v0.8.0은 NH 실주문이 하드락되어 있습니다. Paper Trading 검증 전용입니다.')
