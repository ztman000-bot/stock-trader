...
from backtest_engine import _dt,available_codes,_ema,_vwap
...
EXIT_CONFIGS=[
 {'id':'control_exit','name':'기존 Exit','stop':.010,'trail_act':.015,'trail':.008,'be_act':.008,'be_lock':.0035,'failure_bars':0,'min_mfe':0,'failure_max_return':999,'atr':False,'dynamic_trail':False,'penalty':0},
 {'id':'fast_failure','name':'Fast Failure 15m','stop':.007,'trail_act':.014,'trail':.0065,'be_act':.007,'be_lock':.0025,'failure_bars':3,'min_mfe':.0025,'failure_max_return':.0015,'atr':False,'dynamic_trail':False,'penalty':.003},
 {'id':'mfe_time_stop','name':'MFE Time Stop 20m','stop':.008,'trail_act':.016,'trail':.007,'be_act':.008,'be_lock':.0030,'failure_bars':4,'min_mfe':.0035,'failure_max_return':.0020,'atr':False,'dynamic_trail':False,'penalty':.003},
 {'id':'balanced','name':'Balanced PF','stop':.008,'trail_act':.017,'trail':.0075,'be_act':.009,'be_lock':.0030,'failure_bars':6,'min_mfe':.0035,'failure_max_return':.0025,'atr':False,'dynamic_trail':False,'penalty':.002},
 {'id':'winner_extension','name':'Dynamic Winner Trail','stop':.008,'trail_act':.018,'trail':.0085,'be_act':.010,'be_lock':.0035,'failure_bars':5,'min_mfe':.0030,'failure_max_return':.0020,'atr':False,'dynamic_trail':True,'penalty':.005},
 {'id':'atr_adaptive','name':'ATR Adaptive','stop':.009,'trail_act':.017,'trail':.0075,'be_act':.008,'be_lock':.0030,'failure_bars':5,'min_mfe':.0030,'failure_max_return':.0020,'atr':True,'dynamic_trail':False,'penalty':.005},
]
FILTERS=[
...
ENTRY_MODES=[
 {'id':'signal_open','name':'신호 다음봉 즉시진입','wait':0,'penalty':0},
 {'id':'pullback_support','name':'EMA9/VWAP Pullback 확인','wait':3,'penalty':.006},
]
PLAY_MODES=[
 {'id':'all','name':'Stocks-in-Play 제한 없음','min':None,'penalty':0},
 {'id':'sip60','name':'Stocks-in-Play 60+','min':60,'penalty':.005},
]

def _metrics(xs):
    n=len(xs);w=[x for x in xs if x>0];l=[x for x in xs if x<0];flat=n-len(w)-len(l)
    gp=sum(w);gl=abs(sum(l));pf=gp/gl if gl else (999 if gp else 0);eq=peak=mdd=0
    for x in xs:eq+=x;peak=max(peak,eq);mdd=min(mdd,eq-peak)
    aw=mean(w) if w else 0;al=mean(l) if l else 0;pay=aw/abs(al) if al<0 else (999 if aw>0 else 0)
    return {
        'trades':n,'wins':len(w),'losses':len(l),'flat':flat,
...
def _play_pass(score,mode):return mode['min'] is None or (score is not None and score>=mode['min'])

def _resolve_entry_index(rows,i,mode):
    if mode is None or mode['id']=='signal_open':return i
    if i<=0 or i>=len(rows):return None
    day=_dt(rows[i]['bucket']).date();signal_close=float(rows[i-1]['close']);wait=max(1,int(mode.get('wait') or 3))
    for j in range(i,min(len(rows)-1,i+wait)):
        if _dt(rows[j]['bucket']).date()!=day:break
        hist=rows[max(0,j-100):j+1];cl=[float(x['close']) for x in hist]
        session=[x for x in hist if _dt(x['bucket']).date()==day]
        if len(cl)<20 or len(session)<6:continue
        e9=_ema(cl[-60:],9);vw=_vwap(session)
        if e9 is None or vw is None:continue
        support=max(float(e9),float(vw));lo=float(rows[j]['low']);close=float(rows[j]['close'])
        if lo<=support*1.0025 and close>=support and close<=signal_close*1.008:
            nxt=j+1
...
def _candidates(max_codes=40):
    ...
    for code in codes:
        ...
        for i in sigs:
            ...
            sig_bucket=rows[max(0,i-1)]['bucket'];label=regime.get(sig_bucket,{}).get('label','UNKNOWN');surge=_early_surge_score(rows,i);sip=historical_score(rows,i)
            out.append({'code':code,'date':d,'i':i,'rows':rows,'feat':feat,'regime':label,'surgeScore':surge,'sipScore':(sip or {}).get('score'),'sip':sip,'quality':'GOOD'})
...
def run_profitability_lab(max_codes=40):
                ...
                for sm in SURGE_MODES:
                    for em in ENTRY_MODES:
                        for pm in PLAY_MODES:
                            tr=_eval(cands,f,cfg,rm,sm,train_dates,entry_mode=em,play_mode=pm)
                            if tr['trades']<8:continue
                            complexity=f['penalty']+cfg['penalty']+rm['penalty']+sm['penalty']+em['penalty']+pm['penalty']
                            objective=tr['expectancyPct']*min(1,sqrt(tr['trades']/40))+max(-.05,min(.05,(tr['profitFactor']-1)*.05))-complexity
    ...
    return {
        ...
        'stocksInPlayCoverage':round(sip_known/len(cands)*100,2) if cands else 0,
        'trainDays':len(train_dates),'oosDays':len(oos_dates),
        'best':{
            'strategyId':f['id'],'strategy':f['name'],'exitId':cfg['id'],'exit':cfg['name'],
            'entryModeId':em['id'],'entryMode':em['name'],'marketMode':rm['name'],
            'surgeMode':sm['name'],'stocksInPlayMode':pm['name'],
            'full':full,'oos':oos,'stress2xSlippage':stress,'oneBarLate':late
        },
        ...
        'notes':[
            ...
            'Stocks-in-Play 점수는 Cross와 독립적인 point-in-time 활동성 특징이며 60+ 제한을 Challenger로 비교합니다.',
            '평균 이익/평균 손실/payoff ratio를 PF와 함께 추적합니다.',
            '최적 조합은 개발 구간에서만 선택하며 OOS/비용/지연 스트레스를 별도 확인합니다.'
        ]
    }
