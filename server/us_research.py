...
Purpose: start building US point-in-time research data now, before US Paper/live trading.
- Official NH individual-stock period endpoint: /gbstock/quote/v1/period
- 1-minute regular-session bars are stored and aggregated to 5-minute bars.
- Point-in-time Stocks-in-Play features are recorded without future data.
- SPY/QQQ/IWM are research proxies only, never order candidates.
- US and KR research/statistics stay separate.
- No order endpoint exists in this module.
"""
import math
import os
import sqlite3
...
from us_collector import NY, US_DB_PATH, US_WATCHLIST, us_call, us_regular_session

US_RESEARCH_ENABLED = os.getenv('US_RESEARCH_ENABLED', 'true').lower() == 'true'
...
def init_db():
    with _conn() as c:
        ...
        CREATE TABLE IF NOT EXISTS us_bars_1m(
          ...
          source TEXT NOT NULL DEFAULT 'nh_period_1m',
        ...
        CREATE TABLE IF NOT EXISTS us_bars_5m(
          ...
          volume REAL NOT NULL DEFAULT 0,
          dollar_volume REAL NOT NULL DEFAULT 0,
          source TEXT NOT NULL DEFAULT 'agg_1m',
          PRIMARY KEY(ticker,bucket)
        );
...
def _period_1m(ticker, end_dt, today_only=False):
    payload = {
        'iem_cd': ticker,
        'end_dt': end_dt,
        'count': f'{US_PERIOD_COUNT:04d}',
        'maxavg': '000',
        'gubun': '2',       # minute
        'xtick': '0001',    # 1 minute
        'today_cls': '1' if today_only else '0',
        'market_cls': '1',  # regular session only
    }
    return us_call('/gbstock/quote/v1/period', payload)


def _parse_rows(data, wanted_date=None):
    out = []
    for r in _output1(data):
        ds = str(r.get('trade_date') or r.get('bsop_date') or '').strip()
        ts = str(r.get('trade_time') or '').strip().zfill(6)
        ...
        try:
            ...
            vol = max(0.0, _num(r.get('movolume')) or 0.0)
            value = max(0.0, _num(r.get('movalue')) or cl * vol)
            out.append({'bucket':dt.isoformat(),'session_date':dt.date().isoformat(),
                        'open':o,'high':h,'low':l,'close':cl,'volume':vol,'dollar_volume':value})
        except: continue
...
def _write_1m(ticker, rows):
    ...
    with _conn() as c:
        c.executemany('''INSERT INTO us_bars_1m(ticker,bucket,session_date,open,high,low,close,volume,dollar_volume)
          VALUES(?,?,?,?,?,?,?,?,?)
          ON CONFLICT(ticker,bucket) DO UPDATE SET open=excluded.open,high=excluded.high,low=excluded.low,
          close=excluded.close,volume=excluded.volume,dollar_volume=excluded.dollar_volume''',
...
def _aggregate_5m(ticker, session_date):
    with _conn() as c:
        src = [dict(r) for r in c.execute('SELECT * FROM us_bars_1m WHERE ticker=? AND session_date=? ORDER BY bucket',(ticker,session_date))]
    groups = defaultdict(list)
    for r in src:
        try:
            dt = datetime.fromisoformat(r['bucket']).astimezone(NY)
            b = dt.replace(minute=(dt.minute//5)*5,second=0,microsecond=0).isoformat()
            groups[b].append(r)
        except: pass
    rows=[]
    for b,items in sorted(groups.items()):
        items.sort(key=lambda x:x['bucket'])
        rows.append((ticker,b,session_date,float(items[0]['open']),max(float(x['high']) for x in items),
                     min(float(x['low']) for x in items),float(items[-1]['close']),sum(float(x['volume']) for x in items),
                     sum(float(x['dollar_volume']) for x in items)))
    if rows:
        with _conn() as c:
            c.executemany('''INSERT INTO us_bars_5m(ticker,bucket,session_date,open,high,low,close,volume,dollar_volume)
              VALUES(?,?,?,?,?,?,?,?,?)
              ON CONFLICT(ticker,bucket) DO UPDATE SET open=excluded.open,high=excluded.high,low=excluded.low,
              close=excluded.close,volume=excluded.volume,dollar_volume=excluded.dollar_volume''',rows)
    return len(rows)


def _proxy_map(session_date, ticker):
    with _conn() as c:
        rows=[dict(r) for r in c.execute('SELECT * FROM us_bars_5m WHERE ticker=? AND session_date=? ORDER BY bucket',(ticker,session_date))]
    if not rows: return {}
    op=float(rows[0]['open']);out={}
    for r in rows:
        dt=datetime.fromisoformat(r['bucket']).astimezone(NY)
        out[dt.strftime('%H:%M')] = (float(r['close'])/op-1)*100 if op>0 else 0
    return out
...
def _score(change_pct,gap_pct,rvol,accel,high_prox,above_vwap):
    ...
    change_pts=max(0,min(20,change_pct*5))
    gap_pts=max(0,min(10,gap_pct*2.5))
    high_pts=max(0,min(10,(high_prox-.97)/.03*10))
    vwap_pts=10 if above_vwap else 0
    return round(max(0,min(100,rvol_pts+accel_pts+change_pts+gap_pts+high_pts+vwap_pts)),2)


def _build_snapshots(ticker, target_date):
    if ticker in US_RESEARCH_PROXIES: return 0
    with _conn() as c:
        rows=[dict(r) for r in c.execute('SELECT * FROM us_bars_5m WHERE ticker=? ORDER BY bucket',(ticker,))]
    if not rows: return 0
    spy=_proxy_map(target_date,'SPY');qqq=_proxy_map(target_date,'QQQ');iwm=_proxy_map(target_date,'IWM')
    tod_hist=defaultdict(list);ema9=ema20=None;alpha9=2/10;alpha20=2/21
    day=None;prev_close=None;session=[];cum_pv=cum_vol=0.0;written=[]
    last_close_by_day={}
    for r in rows:
        ...
        if d!=day:
            ...
            prev_close=last_close_by_day.get(day) if day else None
            day=d;session=[];cum_pv=cum_vol=0.0
        ema9=cl if ema9 is None else alpha9*cl+(1-alpha9)*ema9
        ema20=cl if ema20 is None else alpha20*cl+(1-alpha20)*ema20
        if d!=target_date:
            tod_hist[dt.strftime('%H:%M')].append(vol)
            session.append(r)
            continue
        session.append(r)
        op=float(session[0]['open']);hi=max(float(x['high']) for x in session);lo=min(float(x['low']) for x in session)
        ...
        gap=((op/prev_close)-1)*100 if prev_close and prev_close>0 else 0.0
        change=((cl/op)-1)*100 if op>0 else 0.0;range_pct=((hi-lo)/op)*100 if op>0 else 0.0;prox=cl/hi if hi>0 else 0
        orb=max(float(x['high']) for x in session[:6]) if session else None
    ...
    if written:
        with _conn() as c:
            c.executemany('''INSERT INTO us_research_snapshots(ticker,bucket,session_date,gap_pct,change_pct,vwap,ema9,ema20,orb30_high,rvol_tod,volume_accel,dollar_volume,range_pct,high_proximity,stocks_in_play_score,spy_change_pct,qqq_change_pct,iwm_change_pct,created_at)
              VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
              ON CONFLICT(ticker,bucket) DO UPDATE SET gap_pct=excluded.gap_pct,change_pct=excluded.change_pct,vwap=excluded.vwap,
              ema9=excluded.ema9,ema20=excluded.ema20,orb30_high=excluded.orb30_high,rvol_tod=excluded.rvol_tod,volume_accel=excluded.volume_accel,
              dollar_volume=excluded.dollar_volume,range_pct=excluded.range_pct,high_proximity=excluded.high_proximity,stocks_in_play_score=excluded.stocks_in_play_score,
              spy_change_pct=excluded.spy_change_pct,qqq_change_pct=excluded.qqq_change_pct,iwm_change_pct=excluded.iwm_change_pct,created_at=excluded.created_at''',written)
    return len(written)


def _fetch_logged(ticker, session_date):
    with _conn() as c:
        r=c.execute('SELECT status,rows FROM us_research_fetch_log WHERE ticker=? AND session_date=?',(ticker,session_date)).fetchone()
...
def _log_fetch(ticker, session_date, n):
    ...
    with _conn() as c:
        c.execute('''INSERT INTO us_research_fetch_log(ticker,session_date,status,rows,fetched_at) VALUES(?,?,?,?,?)
...
def _refresh_date(session_date, today_only=False, skip_complete=False):
    ...
    for ticker in US_RESEARCH_CODES:
        ...
        if skip_complete:
            ...
            if old and old.get('status') in ('COMPLETE','EMPTY'): continue
        data=_period_1m(ticker,session_date.replace('-',''),today_only=today_only)
        rows=_parse_rows(data,session_date);n=_write_1m(ticker,rows);total1+=n
        total5+=_aggregate_5m(ticker,session_date);_log_fetch(ticker,session_date,n)
        with _LOCK:_STATUS['historyCalls']+=1
    snaps=0
    for ticker in US_WATCHLIST: snaps+=_build_snapshots(ticker,session_date)
    with _LOCK:
        _STATUS['bars1mWritten']+=total1;_STATUS['bars5mWritten']+=total5;_STATUS['snapshotsWritten']+=snaps
    return {'bars1m':total1,'bars5m':total5,'snapshots':snaps}


def _history_dates(n):
    d=datetime.now(NY).date();out=[]
    while len(out)<n:
...
def latest_us_research(limit=20):
    ...
    with _conn() as c:
        rows=[dict(r) for r in c.execute('''SELECT * FROM us_research_snapshots
          WHERE bucket IN (SELECT MAX(bucket) FROM us_research_snapshots GROUP BY ticker)
          ORDER BY stocks_in_play_score DESC LIMIT ?''',(limit,))]
    return rows
...
def research_status():
    init_db()
    with _conn() as c:
        one=c.execute('SELECT COUNT(*),COUNT(DISTINCT session_date) FROM us_bars_1m').fetchone()
        five=c.execute('SELECT COUNT(*),COUNT(DISTINCT session_date) FROM us_bars_5m').fetchone()
        snap=c.execute('SELECT COUNT(*),COUNT(DISTINCT session_date) FROM us_research_snapshots').fetchone()
        latest=c.execute('SELECT MAX(bucket) FROM us_research_snapshots').fetchone()[0]
    ...
    return {**s,'watchlist':list(US_WATCHLIST),'researchProxies':list(US_RESEARCH_PROXIES),'historyDaysTarget':US_HISTORY_DAYS,
            'bars1m':int(one[0]),'bar1mDays':int(one[1]),'bars5m':int(five[0]),'bar5mDays':int(five[1]),
            'snapshots':int(snap[0]),'snapshotDays':int(snap[1]),'latestSnapshot':latest,
            'fetchQuality':quality,'usRegularSession':us_regular_session(),'krRegularSession':kr_regular_session(),
            'researchOnly':True,'paperEnabled':False,'realOrderEnabled':False}
    ...
    while not _STOP.is_set():
        try:
            ...
            if us_regular_session():
                ...
                with _LOCK:
                    _STATUS['liveRefreshes']+=1;_STATUS['lastSuccessAt']=datetime.now(timezone.utc).isoformat();_STATUS['lastError']=None
            ...
            if _backfill_due():
                ...
                with _LOCK:
                    _STATUS['lastBackfillAt']=datetime.now(timezone.utc).isoformat();_STATUS['lastSuccessAt']=_STATUS['lastBackfillAt'];_STATUS['lastError']=None
