"""Market Research Lab v0.16.6. Research only; never changes live rules or sends orders."""
...
def _feature(rows, signal_i):
 # signal_i is next-bar execution index; inspect only the completed signal bar.
...
def _failure_analysis(codes):
 ...
 for code in codes:
  rows,sigs=_signals(code,'cross_trend_v2')
  used=set()
  for i in sigs:
   ...
   pnl=_exit_trade(rows,i,0); rec={**feat,'pnlPct':_f(pnl,3)};trades.append(rec)
   (wins if pnl>0 else losses).append(rec)
   if pnl<=0:
...
def _build_regime(codes):
 # Historical index bars are not cached yet, so use cross-sectional breadth/median-return proxy.
 by=defaultdict(list)
 for code in codes:
 ...
 for b,vals in by.items():
  ...
  breadth=sum(v>0 for v in vals)/len(vals);med=median(vals)
  if breadth<.35 and med<-.50:label='RED'
  elif breadth<.45 or med<-.25:label='CAUTION'
  else:label='NORMAL'
  out[b]={'label':label,'breadth':breadth,'medianRetPct':med,'n':len(vals)}
 return out
...
def _regime_test(codes):
 ...
 return {'method':'BREADTH_PROXY','thresholds':{'red':'breadth < 35% AND median intraday return < -0.50%','caution':'breadth < 45% OR median intraday return < -0.25%'},'groups':{k:met(v) for k,v in buckets.items()},'warning':'현재 로컬 DB에 KOSPI/KOSDAQ 지수 5분봉이 없어 40종목 횡단면 breadth proxy로 연구합니다. 지수 데이터 연결 후 Index Regime으로 교체 예정.'}
...
def _surge_lab(codes):
 ...
 for code in codes:
  ...
  for d,arr in sorted(days.items()):
   arr=sorted(arr,key=lambda x:x['bucket']);regular=[x for x in arr if 540<=(_dt(x['bucket']).hour*60+_dt(x['bucket']).minute)<=930]
   first=[x for x in regular if 540<=(_dt(x['bucket']).hour*60+_dt(x['bucket']).minute)<570]
   after=[x for x in regular if (_dt(x['bucket']).hour*60+_dt(x['bucket']).minute)>=570]
   if len(first)<6 or not after: 
    if regular:prev_close=float(regular[-1]['close'])
    continue
   op=float(first[0]['open']);p30=float(first[-1]['close']);hi=max(float(x['high']) for x in first);lo=min(float(x['low']) for x in first);v30=sum(int(x['volume']) for x in first)
   gap=(op/prev_close-1)*100 if prev_close else 0;openret=(p30/op-1)*100;rvol=v30/prev_first_vol if prev_first_vol else 0;future=(max(float(x['high']) for x in after)/p30-1)*100
   score=0
   if gap>=1:score+=15
   ...
   candidate=score>=60;surge=future>=SURGE_PCT
   rows_out.append({'candidate':candidate,'surge':surge,'score':score,'future':future,'gap':gap,'openret':openret,'rvol':rvol})
   prev_first_vol=v30;prev_close=float(regular[-1]['close'])
...
def run_market_lab(max_codes=40):
 ...
 return {'ok':True,'labVersion':'0.16.6','controlStrategy':'v0.8.0 LOCKED','researchOnly':True,'liveRuleAutoMutation':False,'realOrderEnabled':False,'codesTested':len(codes),'failureAnalysis':_failure_analysis(codes),'marketRegime':_regime_test(codes),'surgeDiscovery':_surge_lab(codes),'recommendation':'RED 장세 회피와 급등 후보 사전선별을 각각 Shadow 검증합니다. PF/기대값 개선이 재현되기 전 Control에는 반영하지 않습니다.'}
