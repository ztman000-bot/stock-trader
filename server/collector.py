...
DEFAULT_WATCHLIST = [x.strip() for x in os.getenv('WATCHLIST', '005930,000660,035420,035720,051910').split(',') if x.strip()]
...
def save_quote(code, payload):
    ...
    with _conn() as conn:
        prev = conn.execute(
            'SELECT cumulative_volume FROM quote_samples WHERE code=? ORDER BY id DESC LIMIT 1',
            (code,),
        ).fetchone()
        vol_delta = 0
        ...
        conn.execute(
            ...
            (
                ...
                q.get('iem_nm'),
                now.isoformat(),
                q.get('hoga_bsop_hour'),
                price,
                float(q.get('askp1') or q.get('askp') or 0),
                float(q.get('bidp1') or q.get('bidp') or 0),
                cum_vol,
                float(q.get('stck_oprc') or 0),
                float(q.get('stck_hgpr') or 0),
                float(q.get('stck_lwpr') or 0),
                float(q.get('prdy_ctrt') or 0),
                int(q.get('total_askp_rsqn') or 0),
                int(q.get('total_bidp_rsqn') or 0),
                float(aux.get('scoring') or 0),
            ),
        )
        ...
        if row is None:
            conn.execute(
                'INSERT INTO bars_5m(code,bucket,open,high,low,close,volume,sample_count) VALUES(?,?,?,?,?,?,?,1)',
                (code, bucket, price, price, price, price, vol_delta),
            )
        else:
            conn.execute(
                '''UPDATE bars_5m
                   SET high=?, low=?, close=?, volume=?, sample_count=sample_count+1
                   WHERE code=? AND bucket=?''',
                (
                    max(float(row['high']), price),
                    min(float(row['low']), price),
                    price,
                    int(row['volume']) + vol_delta,
                    code,
                    bucket,
                ),
            )
...
def bars(code, limit=120):
    ...
    with _conn() as conn:
        rows = conn.execute(
            'SELECT * FROM bars_5m WHERE code=? ORDER BY bucket DESC LIMIT ?',
            (code, limit),
        ).fetchall()
