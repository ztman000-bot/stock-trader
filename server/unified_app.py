...
def _has_open_positions():
    try:
        ...
        try:
            return bool(conn.execute("SELECT 1 FROM paper_trades WHERE status='OPEN' LIMIT 1").fetchone()),None
        finally:
            conn.close()
...
def _run_update():
    try:
        if not UPDATE_SCRIPT.exists():
            raise FileNotFoundError(f'업데이트 스크립트 없음: {UPDATE_SCRIPT}')
        # Launch the batch directly through cmd.exe. This removes the old wscript/VBS dependency.
        flags=getattr(subprocess,'CREATE_NO_WINDOW',0) | getattr(subprocess,'DETACHED_PROCESS',0)
        subprocess.Popen(
            ['cmd.exe','/d','/c',str(UPDATE_SCRIPT)],
            cwd=str(ROOT_DIR),
        ...
        # The updater intentionally kills this server. Do not clear running here; a successful
        # restart creates a fresh process. If it never kills us, expose a timeout diagnostic.
        time.sleep(12)
        with _UPDATE_LOCK:
            if _UPDATE.get('running'):
                _UPDATE['lastError']='업데이트 프로세스가 서버를 재시작하지 못했습니다. TEMP 로그를 확인하세요.'
                _UPDATE['running']=False
    except Exception as exc:
        with _UPDATE_LOCK:
            _UPDATE['running']=False
            _UPDATE['lastError']=f'{type(exc).__name__}: {exc}'
...
def update_status(request):
    return JSONResponse({'ok':True,'uiVersion':UI_VERSION,'update':dict(_UPDATE)})

def ui_health(request):
    return JSONResponse({'ok':True,'uiVersion':UI_VERSION,'scannerPolicy':{'safeUniverse':180,'activeFocus':40,'top':10},'validation':{'mfeMae':True,'entrySnapshots':True,'scoreBuckets':True,'profitFactor':True,'expectancy':True,'mdd':True,'failureClassification':True,'controlStrategy':'v0.8.0'},'backtest':{'enabled':True,'controlStrategy':'v0.8.0 LOCKED','costsIncluded':True,'liveMutation':False},'usMarket':{'collector':True,'paper':False,'realOrder':False},'updater':{'launcher':'cmd-direct','wscriptRequired':False,'openPositionGuard':True,'dirtyTreeGuard':True}})
...
app.router.routes.extend([
    ...
    Route('/api/system/update/status',update_status),Route('/api/system/ui-health',ui_health),
