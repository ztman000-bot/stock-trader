...
BASE_DIR=Path(__file__).resolve().parent;ROOT_DIR=BASE_DIR.parent;DASHBOARD=BASE_DIR/'unified_dashboard.html';CLASSIC_INDEX=ROOT_DIR/'index.html';UPDATE_SCRIPT=BASE_DIR/'remote_update.cmd';UI_VERSION='0.16.3';_UPDATE={'running':False,'requestedAt':None,'lastError':None,'launcher':'cmd-direct'};_UPDATE_LOCK=threading.Lock()
...
def ui_health(request):return JSONResponse({'ok':True,'uiVersion':UI_VERSION,'strategyLab':{'enabled':True,'version':'0.16.3','control':'v0.8.0 LOCKED','crossTrend':True,'overnightLab':True,'liveMutation':False},'backtest':{'enabled':True,'engine':'precise-portfolio-v1','fidelity':'portfolio-high','executionModel':'next-bar-open','portfolioConstraints':True,'costsIncluded':True,'liveMutation':False},'usMarket':{'collector':True,'paper':False,'realOrder':False}})
