...
BASE_DIR=Path(__file__).resolve().parent;ROOT_DIR=BASE_DIR.parent;DASHBOARD=BASE_DIR/'unified_dashboard.html';CLASSIC_INDEX=ROOT_DIR/'index.html';UPDATE_SCRIPT=BASE_DIR/'remote_update.cmd';UI_VERSION='0.15.9';_UPDATE={'running':False,'requestedAt':None,'lastError':None,'launcher':'cmd-direct'};_UPDATE_LOCK=threading.Lock()
...
def update_request(request):
 ...
 threading.Thread(target=_run_update,daemon=True).start();return JSONResponse({'ok':True,'message':'업데이트를 시작했습니다. 서버 재시작 후 자동으로 새 버전을 확인합니다.','target':'latest-main'})
...
def history_start(request):
 try:
  days=int(request.query_params.get('days','20'));codes=int(request.query_params.get('max_codes','40'));result=history_start_job(days,codes)
...
def ui_health(request):return JSONResponse({'ok':True,'uiVersion':UI_VERSION,'scannerPolicy':{'safeUniverse':180,'activeFocus':40,'top':10},'validation':{'mfeMae':True,'entrySnapshots':True,'scoreBuckets':True,'profitFactor':True,'expectancy':True,'mdd':True,'failureClassification':True,'controlStrategy':'v0.8.0'},'backtest':{'enabled':True,'engine':'precise-portfolio-v1','fidelity':'portfolio-high','executionModel':'next-bar-open','intrabarPolicy':'stop-first-conservative','portfolioConstraints':True,'historicalAccumulator':True,'maxDays':120,'maxCodes':100,'resumableCache':True,'diagnostics':True,'controlStrategy':'v0.8.0 LOCKED','costsIncluded':True,'liveMutation':False,'sampleWarningBelow':200,'knownLimitations':['historical activity/breadth not reconstructed','current-universe survivorship bias']},'usMarket':{'collector':True,'paper':False,'realOrder':False},'updater':{'launcher':'cmd-direct','wscriptRequired':False,'openPositionGuard':True,'dirtyTreeGuard':True,'compatRoute':True}})
