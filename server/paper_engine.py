...
def _wilder_adx(rows, period=14):
    ...
    tr_s=sum(trs[:period]); pdm_s=sum(pdms[:period]); mdm_s=sum(mdms[:period])
    dxs=[]; pdi=mdi=0.0
    for i in range(period, len(trs)):
        ...
        if tr_s <= 0:
            ...
            pdi=100*pdm_s/tr_s; mdi=100*mdm_s/tr_s
        denom=pdi+mdi
...
def _vwap(rows):
    ...
    if total_v <= 0:
        return None
    pv = sum(((float(r['high'])+float(r['low'])+float(r['close']))/3.0) * max(0, int(r['volume'])) for r in rows)
    return pv / total_v
...
def indicators(code):
    ...
    prev_high=max(float(r['high']) for r in prior6)
    ...
    dmi_ok=adx is not None and adx>=20 and pdi is not None and mdi is not None and pdi>mdi
    volume_up=volume_ratio>=1.20
    strong_volume=volume_ratio>=STRONG_VOLUME_RATIO
    trend_gate=above_vwap and ema_bull
    trigger_gate=breakout or strong_volume
    score=0
    if above_vwap: score+=20
    if ema_bull: score+=20
    score+=max(0,min(15,(volume_ratio-1.0)*18.75)) if volume_ratio>1 else 0
    if breakout:
        breakout_pct=(price/prev_high-1)*100 if prev_high>0 else 0
        score+=min(20,12+breakout_pct*80)
    if rsi_ok: score+=10
    if dmi_ok:
        score+=min(15,8+max(0,adx-20)*0.35)
    score=round(min(100,score),2)
    checks={
        'above_vwap':above_vwap,'ema_bull':ema_bull,'volume_up':volume_up,
        'strong_volume':strong_volume,'breakout':breakout,'rsi_ok':rsi_ok,
        'dmi_ok':dmi_ok,'trend_gate':trend_gate,'trigger_gate':trigger_gate,
    }
    return {
...
def evaluate(code):
    ...
    if c['trend_gate'] and c['trigger_gate'] and score>=BUY_SCORE:
        action='BUY_CANDIDATE'
    elif c['trend_gate'] and (score>=60 or c['volume_up']):
        action='SETUP'
    else:
        action='WATCH'
    if stats['locked'] and action=='BUY_CANDIDATE': action='SHADOW_ONLY'
    reasons=[k for k,v in c.items() if v]
...
def paper_enter(code, capital=INITIAL_CAPITAL):
    ...
    shadow=ev['action']=='SHADOW_ONLY'; price=float(ev['indicators']['price'])
    capital=max(0.0,float(capital)); risk_cash=capital*RISK_PER_TRADE
    risk_qty=math.floor(risk_cash/(price*STOP_PCT)) if price>0 else 0
    ...
    with _conn() as conn:
        ...
        if shadow:
            return {'ok':True,'shadow':True,'message':'2연속 손실 잠금: 신호만 기록, 신규 Paper 진입 없음','evaluation':ev}
        ...
        cur=conn.execute("INSERT INTO paper_trades(code,entry_at,entry_price,qty,score,reasons,status,peak_price) VALUES(?,?,?,?,?,?, 'OPEN',?)",(code,now,entry_fill,qty,ev['score'],reasons,entry_fill))
...
def mark_positions():
    init_paper_db(); latest={r['code']:float(r['price']) for r in latest_quotes()}; closed=[]
    with _conn() as conn:
        ...
        for p in positions:
            ...
            if reason:
                ...
                pnl=gross-fees; pnl_pct=pnl/(entry*qty)*100 if entry*qty else 0
                now=datetime.now(KST).isoformat()
                conn.execute("UPDATE paper_trades SET exit_at=?,exit_price=?,exit_reason=?,pnl=?,pnl_pct=?,status='CLOSED',peak_price=? WHERE id=?",(now,exit_fill,reason,pnl,pnl_pct,peak,p['id']))
                closed.append({'id':p['id'],'code':p['code'],'reason':reason,'pnl':round(pnl,2),'pnlPct':round(pnl_pct,4)})
