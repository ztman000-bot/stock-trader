...
def _summary(m,p,hs,us):
 b=p.get('best') or {};rd=p.get('readiness') or {};full=b.get('full') or {};oos=b.get('oos') or {};stress=b.get('stress2xSlippage') or {};wf=p.get('walkForward') or {};lock=p.get('lockbox') or {}
 lines=[f"자동 연구 데이터: {hs.get('writtenBars',0):,}개 갱신, Universe Snapshot {us.get('snapshotDays',0)}일/{us.get('snapshotRows',0)}행."]
 if b:lines.append(f"선두 {b.get('strategy')} + {b.get('exit')}: {full.get('trades',0)}건, PF {_fmt(full.get('profitFactor'))}, 기대값 {_fmt(full.get('expectancyPct'),3)}%. OOS PF {_fmt(oos.get('profitFactor'))}, 2x 슬리피지 PF {_fmt(stress.get('profitFactor'))}.")
 if wf:lines.append(f"Walk-Forward {wf.get('folds',0)}구간 중 통과 {wf.get('positiveFolds',0)}구간. Final Lockbox PF {_fmt(lock.get('profitFactor'))}, 기대값 {_fmt(lock.get('expectancyPct'),3)}%.")
 lines.append(f"실전 준비도 {rd.get('score',0)}/100. 최종판정: {'연구 게이트 통과' if rd.get('pass') else '실전 승격 금지'}. Control v0.8.0 LOCKED · 자동 전략변경 OFF · REAL ORDER OFF.")
 return '\n\n'.join(lines)
def _history_due():
...
def run_once():
 with _LOCK:
  ...
  _STATE['running']=True
 try:
  hs=_accumulate_and_wait() if _STATE['historyAuto'] else history_status();_STATE['phase']='universe-snapshot';us=_snapshot_universe();_STATE['phase']='strategy-backtest';s=run_lab(40);_STATE['phase']='market-research';m=run_market_lab(80);_STATE['phase']='profitability';p=run_profitability_lab(40)
  data={'ok':True,'generatedAt':datetime.now().isoformat(timespec='seconds'),'summary':_summary(m,p,hs,us),'history':hs,'universeSnapshot':us,'market':m,'strategy':s,'profitability':p,'safety':{'control':'v0.8.0 LOCKED','liveMutation':False,'realOrder':False,'liveSessionPriority':True}}
